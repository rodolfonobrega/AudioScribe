...
def load_config(path: Optional[str] = None, use_env: bool = False, **kwargs) -> Config:
    ...
    if use_env:
        ...
        config.transcription.model = os.getenv('TRANSCRIPTION_MODEL', config.transcription.model)
        config.transcription.api_key = os.getenv('TRANSCRIPTION_API_KEY', config.transcription.api_key) or os.getenv('GROQ_API_KEY')
